...
class MainWindow(qtw.QWidget):
    ...
    def func_result(self):
        fin_string = ''.join(self.fin_nums).lstrip('0') + ''.join(self.temp_nums).lstrip('0')
        if fin_string != '':
            if fin_string[len(fin_string) - 1] != '+' and fin_string[len(fin_string) - 1] != '-' and fin_string[len(fin_string) - 1] != '*' and fin_string[len(fin_string) - 1] != '/':
